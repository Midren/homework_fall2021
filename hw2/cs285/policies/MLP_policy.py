...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        loss = -(self.forward(observations).log_prob(actions)*advantages).mean()
        loss.backward()
        self.optimizer.step()

        if self.nn_baseline:
            self.baseline_optimizer.zero_grad()
            baseline_loss = self.baseline_loss(self.baseline.forward(observations), ptu.from_numpy((q_values - np.mean(q_values))/np.std(q_values)))
